...
def evaluate_and_export(model, dataset, subset_name, args):
    ...
    predictions, eval_loss = evaluate(
        ...
        is_test=True,
        use_san=args.use_san,
        use_lel=args.use_lel,
    )
    ...
    predictions_df = predictions_df.join(dataset_df)
...
def load_model_from_pretrained(
    ...
    is_lel_model = "ltn" in model_type or "lel" in model_type
    ...
    num_labels = len(labelmaps["id2label"]) if labelmaps else len(model.config.id2label)
    ...
    if replace_classification:
        ...
        elif isinstance(model, (LTNRoberta)):
            pass
        else:
...
def train(model, tokenizer, optimizer, scheduler, train_dataset, val_dataset, args):
    ...
    collate_fn = partial(
        data_iterators.collate_fn, add_san_masks=args.use_san, add_lel_masks=args.use_lel
    )
...
def evaluate_with_metrics(
    model, dataset, batch_size, is_test=False, num_workers=0, use_san=False, use_lel=False
):
    predictions, eval_loss = evaluate(
        model,
        dataset,
...
def main():
    ...
    args.use_san = False
    args.use_lel = "ltn" in args.model_type or "lel" in args.model_type
